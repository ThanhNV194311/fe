...
export const getAllProduct = () => async dispatch => {
	try {
		const { data } = await axios.get(
			`https://be-production-05ac.up.railway.app/products/`
		);
...
export const getproductById = id => async dispatch => {
	try {
		const { data } = await axios.get(
			`https://be-production-05ac.up.railway.app/products/detail/${id}`
		);
...
export const saveProduct = product => async (dispatch, getState) => {
	try {
		...
		if (!product.get("_id")) {
			const { data } = await axios.post(
				"https://be-production-05ac.up.railway.app/products/create",
				product,
				{
					headers: {
			...
			const { data } = await axios.put(
				`https://be-production-05ac.up.railway.app/products/update`,
				product,
				{
					headers: {
						Authorization: `Bearer ${userInfo.token}`,
...
export const DeleteProduct = productId => async (dispatch, getState) => {
	try {
		...
		const { data } = await axios.delete(
			`https://be-production-05ac.up.railway.app/products/delete/${productId}`,
			{
				headers: {
					Authorization: `Bearer ${userInfo.token}`,
				},
...
export const searchProduct = name => async (dispatch, getState) => {
	try {
		const { data } = await axios.get(
			`https://be-production-05ac.up.railway.app/products/search/product?name=${name}`
		);
...
export const reviewProduct = (id, review) => async (dispatch, getState) => {
	try {
		const { data } = await axios.post(
			`https://be-production-05ac.up.railway.app/products/rate/${id}`,
			review
		);
...
export const commentProduct = (id, comment) => async (dispatch, getState) => {
	try {
		const { data } = await axios.post(
			`https://be-production-05ac.up.railway.app/products/comment/${id}`,
			comment
		);
...
export const repCommentProduct =
	(id, comment) => async (dispatch, getState) => {
		try {
			const { data } = await axios.post(
				`https://be-production-05ac.up.railway.app/products/rep/comment/${id}`,
				comment
			);
...
export const pinCommentProduct =
	(id, comment) => async (dispatch, getState) => {
		try {
			const { data } = await axios.post(
				`https://be-production-05ac.up.railway.app/products/pin/comment/${id}`,
				comment
			);
...
export const BlogProduct =
	(id, blog, callback) => async (dispatch, getState) => {
		...
		try {
			const { data } = await axios.post(
				`https://be-production-05ac.up.railway.app/products/blog/${id}`,
				blog,
				{
					headers: {
						Authorization: `Bearer ${userInfo.token}`,
