...
function AppChat(props) {
	const ENDPOINT = "https://be-production-05ac.up.railway.app";
	...
	useEffect(() => {
		const getAllMessageByConversation = async () => {
			const { data } = await axios.get(
				`https://be-production-05ac.up.railway.app/chat/message?idUser=${userInfo._id}`
			);
			...
			{openChat ? (
				<div className="chatuser">
					<div className="chatuser-user">
						<span className="chatuser-user-name">Chăm sóc khách hàng</span>
						<span
