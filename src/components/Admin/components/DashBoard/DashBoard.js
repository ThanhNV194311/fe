	...
	useEffect(() => {
		const fetch = async () => {
			const { data } = await axios.post(
				"https://be-production-05ac.up.railway.app/order/findInfo",
				{
			...
			form.setFieldsValue({
				start: dateSearch.start ? dayjs(dateSearch.start, "YYYY-MM-DD") : null,
				end: dateSearch.end ? dayjs(dateSearch.end, "YYYY-MM-DD") : null,
			});
			setValuePage(data);
		};
		fetch();
				...
				>
					...
					<Col span={6}>
						...
						>
							...
							<span>
								<strong style={{ marginRight: 10 }}>
									{VND.format(handleTotal(valuePage?.order?.list))}
								</strong>
							</span>
						</div>
					</Col>
